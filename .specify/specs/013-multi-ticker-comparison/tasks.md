...
- [X] **T001** `TICKER_UNIVERSE` — explicit named constant,
  `["AAPL", "MSFT", "GOOGL", "NVDA", "AMZN"]`. Already decided; not a
  placeholder. (FR-001)
- [X] **T002** `run_one_ticker` — full pipeline composition
...
- [X] **T003** `run_comparison` — loops every ticker independently; one
  failure never stops the rest; returns `(results_frame, failures)`.
  (FR-003, FR-004)
- [X] **T004** Mandatory honesty columns: `Median hurdle (bps)` and
...
- [X] **T005** Write output via `data.cache_path` as one CSV with a
...
- [X] **T006** End-to-end `main()` three-way comparison per ticker
  (cost-aware ML vs. buy-and-hold vs. random). **Deviation from this task's
  original text, recorded in `plan.md`**: baseline rows are built by calling
  `signals.buy_and_hold_signal`/`random_signal` directly and summarizing with
  `metrics.performance_summary`, not by reusing
  `ma_crossover_backtest.baseline_results` — that function summarizes with
  `backtest_harness.summarize_trades`, which carries no Sharpe or drawdown,
  and User Story 2 requires the baseline rows to be risk-adjusted too.
  `mean_holding_bars` is reused verbatim.

## Phase 3 — Tests (Rule 5, FR-009), synthetic/network-free data

- [X] **T007** [P] Isolated-failure case: one ticker too short for a fold;
  others complete; failure named with reason. (SC-001)
- [X] **T008** [P] All-succeed case: one row per (ticker, strategy), three
  strategies per ticker. (SC-002)
- [X] **T009** [P] All-fail case: runner completes and reports every
  failure rather than raising on the first. 
- [X] **T010** [P] Cost-parameter consistency across every row. (SC-003)
- [X] **T011** [P] CSV round-trip via `cache_path`. (SC-004)

## Phase 4 — Evidence

- [X] **T012** Mutation check — at minimum: let one ticker's exception
  propagate and stop the loop; silently drop a baseline for one ticker.
  Each must fail at least one test. (SC-005)
- [X] **T013** Full suite passes; record the new total in this file's PR.
  **416 tests, OK** (was 405 before this spec; 11 added).
